...
BadgeObject.prototype.toString = function () {
  var report = "";
  rp("\n================== Validation Report for Badge Object: " + this.id + " ==================");
  
  rp("\nWarnings (" + this.warnings.length + "): \n");
  for (var i=0; i<this.warnings.length; i++){
    rp("  * " + clc.yellow(this.warnings[i].message) + "\n");
  }

  rp("\nErrors (" + this.errors.length + "): \n");
  for (i=0; i<this.errors.length; i++){
    rp("  * " + clc.red(this.errors[i].message) + "\n");
  }
  if (this.structures){
    rp("\nStandard validations run: (" + this.structures.length + "): \n");
    for (i=0; i<this.structures.length; i++){
      rp(clc.cyan( JSON.stringify(this.structures[i]) ) + "\n");
    } 
  }
  if(this.report){
          ...
          analyzeBadgeObjectForValidation(badgeObject, function (err, badgeInfoObject){
            ...
            function validatedOne(){
              ...
              if (validatedCount === structures.length){
                badgeObject = _.merge(badgeObject,badgeInfoObject);
                callback(badgeObject);
              }
            }
  ...
  async.waterfall([
    ...
    function (callback){
      ...
      // or the more standard case that we're analyzing an assertion and have now fetched the badgeclass
        ...
        infoBlock.warnings.push(new Warning("The Issuer file was linked from a remote resource and not embedded in the verified assertion. There's a slight possibility that the Issuer may have been modified from its original version."));
