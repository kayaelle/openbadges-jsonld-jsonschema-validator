...
function analyzeBadgeObjectForValidation (badgeObject, readyToValidate){
  ...
  function fetchBadgeObject(url, callback) {
    ...
    fetch(url, function (err, fetchedObject) {
      console.log("I've fetched something maybe! [" + url + ", " + "\n\n" + clc.green(JSON.stringify(fetchedObject,null,"  ")));
      if (err){
        callback(err, null);
      }
      else {
  ...
  async.waterfall([
    ...
    function (callback){
      if (typeOfObject.match(/assertion/i) && typeof data.badge === 'string') {
        ...
        console.log("FETCHING BADGE: " + data.badge);
    ...
    function (callback){
      ...
      if(typeOfObject.match(/badgeclass/i) && typeof data.issuer === "string"){
        console.log("FETCHING ISSUER: " + data.issuer);
      ...
      // or the more standard case that we're analyzing an assertion and have now fetched the badgeclass
        ...
        console.log("FETCHING ISSUER: " + infoBlock.full.badge.issuer);
  ...
  function (err) {
    if (err){ 
      console.log("BLAST! There was an error: " + err);
      // kick back out to the top level.
      return;
    }
    if (infoBlock.fullExpanded)
        console.log("FULL EXPANDED, DAWG!!! \n" +
        clc.yellow(JSON.stringify(infoBlock.fullExpanded,null,"  ")) + 
          "\n\n===================================================="); 
    // TODO: generate types and put them into this block.
    ...
    async.map(contextBlocks,analyzeContextBlock,function (err, results){
      if (err)
        console.log("ACK. An error has bubbled up to here! " + err);
      else{
        console.log("RESUUUUUUULTS: " + clc.magenta(JSON.stringify(results)));
        infoBlock.structures = infoBlock.structures.concat(results);
        console.log("The resulting structures are: \n" + infoBlock.structures);
      }
  ...
  function analyzeContextBlock(block, nextBlock){
    ...
    if (typeof block.context === 'string'){
      getSchemaForContext(block.context, block.type, function (err, schemaRef){
        if (err) {
          console.log("Couldn't get schema reference for context doc: " + block.context); 
    ...
    // Case 2: the extended format needs a few passes.
      ...
      async.map(
          ...
          if (typeof curContext === 'string') {
            getSchemaForContext(curContext, block.type, function (err, schemaRef){
              if (err) {
                ...
                nextElement(null, { pointer: block.pointer, contextRef: block.context, schemaRef: schemaRef });
              }
            });
          }
        ...
        function (err, results){
          if (err) {
            console.log ("ERMEHGERD IT'S AN ERROR: " + err);
            nextBlock(err);
          }
          else{
            console.log ("I have analized a block! WHAAAAT!? Results here: "+ results);
